...
/**
 * Per-model property overrides. A write here changes how every device of that
 * model is polled and commanded, so it is the smallest surface with the widest
 * blast radius in the service.
 */
describe('ModelPropertyOverridesController (integration)', () => {
    ...
    describe('Authentication', () => {
        const routes: ReadonlyArray<readonly [string, string]> = [
            ['get', '/model-property-overrides'],
            ['post', '/model-property-overrides'],
            ['delete', '/model-property-overrides/671b00000000000000000001']
        ];
        ...
        describe('the refusals that separate a token from a token for us', () => {
            const probe = () => request.get('/model-property-overrides');
